...
export function SelectCell<TData>({
  ...
  const containerRef = React.useRef<HTMLDivElement>(null);
  const cellOpts = cell.column.columnDef.meta?.cell;
  const options = React.useMemo<Array<{ value: string; label: string }>>(
    () => (cellOpts?.variant === "select" ? (cellOpts.options as Array<{ value: string; label: string }>) : []),
    [cellOpts],
  );
  const optionByValue = React.useMemo(
    () => new Map<string, { value: string; label: string }>(options.map((option) => [option.value, option])),
    [options],
  );
...
export function MultiSelectCell<TData>({
  ...
  const cellOpts = cell.column.columnDef.meta?.cell;
  const options = React.useMemo<Array<{ value: string; label: string }>>(
    () => (cellOpts?.variant === "multi-select" ? (cellOpts.options as Array<{ value: string; label: string }>) : []),
    [cellOpts],
  );
  const optionByValue = React.useMemo(
    () => new Map<string, { value: string; label: string }>(options.map((option) => [option.value, option])),
