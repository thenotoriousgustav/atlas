...
import { Check, Plus, Minus, Play, Pause, NotePencil } from '@phosphor-icons/react';
...
export function QuickCheckinDialog({
  ...
  useEffect(() => {
    if (activeTracker) {
      if (activeTracker.todayEntry) {
        ...
        form.setFieldValue('value', activeTracker.type === 'BOOLEAN' ? 1 : activeTracker.goalValue || 1);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[440px] bg-background border-border shadow-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            Log today&apos;s habit progress cleanly and quickly.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="p-3 rounded-lg border border-border bg-card/50 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-mono text-muted-foreground">
                    Goal: {activeTracker.goalValue} {activeTracker.goalUnit || 'unit'} ({activeTracker.goalDirection})
                  </span>
                </div>
